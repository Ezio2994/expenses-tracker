...
headerButtons[0].onclick = () =>
  headerButtons[0].value === "signIn" ? signIn() : signOut();
...
const updateRef = () => {
  ...
  fixedRef = db
    .collection("users")
    .doc(user)
    .collection("Fixed Incomes-Expenses")
    .doc(date);
  dailyExpensesRef = db
    .collection("users")
    .doc(user)
  ...
  db.collection("users")
    ...
    .then((querySnapshot) => {
      ...
      document.querySelector("#reportSum").innerHTML = `
        Total this year: Incomes: £${incomes.reduce(
          (a, b) => a + b
        )} - Expenses: £${expenses.reduce(
        (a, b) => a + b
      )} - Saved: £${saved.reduce((a, b) => a + b)}`;
    })
    .catch((error) => null);
};
...
document.querySelector("#submitReport").addEventListener("click", () => {
  ...
  showMonth.innerHTML = `${
    months[
      reportsList.value.charAt(1) !== "-"
        ? reportsList.value.slice(0, 2) - 1
        : reportsList.value.charAt(0) - 1
    ]
  } - ${year}`;
  list.innerHTML = "";
  moreList.innerHTML = "";
...
const addData = () => {
  ...
  let total = dataBaseFixed
    ? dataBaseFixed.salary + dataBaseFixed.others
    : dataBaseFixed.salary;
  const moreIncomes =
    dataBaseDailyIncomes !== undefined &&
    Object.values(dataBaseDailyIncomes).length
      ? Object.values(dataBaseDailyIncomes).reduce((a, b) => a + b)
      : 0;
  ...
  if (dataBaseDailyExpenses) {
    const dailyIE = Object.values(dataBaseDailyExpenses).length
      ? Object.values(dataBaseDailyExpenses).reduce((a, b) => a + b)
      : 0;
...
addMore.addEventListener("click", (e) => {
  ...
  if (newDataInput.value !== "") {
    newDataInput.value = newDataInput.value.includes("/")
      ? newDataInput.value.replace("/", "-")
      : newDataInput.value;
    moreList.innerHTML += `
            <article>
...
const updateInputs = () => {
  ...
  if (dataBaseDailyExpenses) {
    for (let i = 0; i < Object.keys(dataBaseDailyExpenses).length; i++) {
      dailyExpensesList.innerHTML += `
        <p><button value="${
          Object.keys(dataBaseDailyExpenses)[i]
        }"><i class="fas fa-minus-circle"></i></button> ${
        Object.keys(dataBaseDailyExpenses)[i]
      }: £<input class="displayValues" type="number" readonly value="${
        Object.values(dataBaseDailyExpenses)[i]
      }" name="${Object.keys(dataBaseDailyExpenses)[i]}" id="display${
        Object.keys(dataBaseDailyExpenses)[i]
      }"></p>`;
    }
  }
  ...
  if (dataBaseDailyIncomes) {
    for (let i = 0; i < Object.keys(dataBaseDailyIncomes).length; i++) {
      dailyIncomesList.innerHTML += `
        <p><button value="${
          Object.keys(dataBaseDailyIncomes)[i]
        }"><i class="fas fa-minus-circle"></i></button> ${
        Object.keys(dataBaseDailyIncomes)[i]
      }: £<input class="displayValues" type="number" readonly value="${
        Object.values(dataBaseDailyIncomes)[i]
      }" name="${Object.keys(dataBaseDailyIncomes)[i]}" id="display${
        Object.keys(dataBaseDailyIncomes)[i]
      }"></p>`;
    }
  }
...
const deleteValue = (e, whichRef) => {
  let reference;
  if (whichRef === "expenses") {
    reference = dailyExpensesRef;
  } else if (whichRef === "incomes") {
    reference = dailyIncomesRef;
  } else {
    reference = fixedRef;
  }
...
addNewDailyInputs[2].addEventListener("click", (e) => {
  ...
  const isExpensesObj =
    addNewDailyHeader.innerHTML === "Add Daily Expenses"
      ? dataBaseDailyExpenses
      : dataBaseDailyIncomes;
  ...
  if (isObject.includes(addNewDailyInputs[0].value)) {
    isExpensesRef.update({
      [addNewDailyInputs[0].value]:
        Number(addNewDailyInputs[1].value) +
        Number(isExpensesObj[[addNewDailyInputs[0].value]]),
    });
  } else {
...
closeWindow.forEach((button) =>
  button.addEventListener("click", () => (inputDatas.style.display = "none"))
);
